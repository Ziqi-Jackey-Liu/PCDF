...
class Model(nn.Module):
    ...
    """
    def __init__(self, enc_in, dec_in, c_out, label_len, pred_len, kernel_size=6, d_model=128, d_ff=48):
        ...
        self.enc_embedding = DataEmbeddingWoPos(enc_in, d_model, embed_type='fixed', freq='h')
        self.dec_embedding = DataEmbeddingWoPos(dec_in, d_model, embed_type='fixed', freq='h')

        # Encoder
        self.encoder = Encoder(
            [
                EncoderLayer(
                    AutoCorrelationLayer(AutoCorrelation(factor=3), d_model, n_heads=1),
                    d_model, d_ff, moving_avg=4) for l in range(e_layers)
            ],
        ...
        self.decoder = Decoder(
            [
                DecoderLayer(
                    AutoCorrelationLayer(AutoCorrelation(factor=3), d_model, n_heads=1),
                    AutoCorrelationLayer(AutoCorrelation(factor=3), d_model, n_heads=1),
                    d_model, c_out, d_ff, moving_avg=4,
                )
...
class SeriesDecom(nn.Module):
    """
    Series decomposition block
    """
    def __init__(self, kernel_size):
        super(SeriesDecom, self).__init__()
        self.moving_avg = MovingAvg(kernel_size, stride=1)
...
class EncoderLayer(nn.Module):
    ...
    """
    def __init__(self, attention, d_model, d_ff=None, moving_avg=25, dropout=0, activation="gelu"):
        ...
        if isinstance(moving_avg, list):
            ...
            self.decomp1 = SeriesDecom(moving_avg)
            self.decomp2 = SeriesDecom(moving_avg)
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, self_attention, cross_attention, d_model, c_out, d_ff=None,
        ...
        if isinstance(moving_avg, list):
            ...
            self.decomp1 = SeriesDecom(moving_avg)
            self.decomp2 = SeriesDecom(moving_avg)
            self.decomp3 = SeriesDecom(moving_avg)
...
class FullAttention(nn.Module):
    def __init__(self, factor =1, mask_flag=True, scale=None, attention_dropout=0.1, output_attention=False):
